...
def draw_radar_chart(profiles: Dict[str, CountryProfile]) -> None:
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#0f172a",
        font={"color": "#94a3b8"},
        legend={"font": {"color": "#f1f5f9"}, "bgcolor": "rgba(0,0,0,0)"},
        polar={
            "bgcolor": "#1e293b",
            "radialaxis": {
                "range": [0, 1],
...
def draw_country_setup(
    nations: List[str],
    countries_dir: Path = None,
) -> Dict[str, CountryProfile]:
    """
    Render the country setup page. Returns a dict of nation -> CountryProfile
    reflecting the user's current slider values. Profiles persist in
    st.session_state between reruns.
    """
    st.markdown("## Country Configuration")
    st.caption(
        "Configure each nation's profile before starting the simulation. These parameters scale starting resources, agent capability, and simulation dynamics."
    )
    ...
    for column, nation in zip(columns, nations):
        ...
        with column:
            badge_bg = "#1d4ed8" if nation == "Auria" else "#92400e"
            badge_text = "A" if nation == "Auria" else "B"
            st.markdown(
                f"""
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
                  <span style="display:inline-block;padding:2px 8px;border-radius:999px;background:{badge_bg};color:#f1f5f9;font-size:12px;font-weight:600;">{badge_text}</span>
                  <span style="font-size:1.5rem;font-weight:600;color:#f1f5f9;">{default_profile.display_name}</span>
                </div>
                """,
            ...
            st.slider(
                ...
                max_value=1.0,
                step=0.05,
                key=_profile_key(nation, "gdp_index"),
                help="Economic output. Scales repair crew stocks, medical and food resupply.",
            )
            st.slider(
                "Military Strength",
                min_value=0.0,
                max_value=1.0,
                step=0.05,
                key=_profile_key(nation, "military_strength"),
                help="Logistics and engineering capacity. Unlocks extra agent actions per turn.",
            )
            st.slider(
                "Population (millions)",
                min_value=0.5,
                max_value=50.0,
                step=0.5,
                key=_profile_key(nation, "population_millions"),
                help="Total national population. Scales zone populations and displacement KPIs.",
            )
            st.slider(
                "Resource Richness",
                min_value=0.0,
                max_value=1.0,
                step=0.05,
                key=_profile_key(nation, "resource_richness"),
                help="Natural resource endowment. Scales fuel, generators and water purifier stocks.",
            )
            st.slider(
                "Terrain Difficulty",
                min_value=0.0,
                max_value=1.0,
                step=0.05,
                key=_profile_key(nation, "terrain_difficulty"),
                help="Geographic obstacle level. Reduces transport hub health and increases transport dependency penalties.",
            )
            st.slider(
                "Alliance Strength",
                min_value=0.0,
                max_value=1.0,
                step=0.05,
                key=_profile_key(nation, "alliance_strength"),
                help="External support network. Provides bonus resupply per turn and reduces surprise event probability.",
            )
            ...
            st.markdown(
                (
                    ...
                    f'<span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">GDP: {current_profile.gdp_label()}</span>'
                    f'<span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Military: {current_profile.military_label()}</span>'
                    f'<span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Terrain: {current_profile.terrain_label()}</span>'
                    f'<span style="font-size:11px;color:#94a3b8;text-transform:uppercase;">Alliance: {current_profile.alliance_label()}</span>'
                    '</div>'
                ),
                unsafe_allow_html=True,
            )
    ...
    if len(nations) >= 2:
        ...
        for factor, left_value, right_value in factors:
            ...
            comparison_rows.append({
                "Factor": factor,
                f"{nations[0]} value": round(left_value, 2),
                f"{nations[0]} label": _factor_label(left, factor),
                f"{nations[1]} value": round(right_value, 2),
                f"{nations[1]} label": _factor_label(right, factor),
                "Advantage": advantage,
            })
