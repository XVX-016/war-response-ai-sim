...
export default function CapabilityCards() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {CAPS.map((c) => (
        <div key={c.title} className="border border-[#333333] rounded p-8 hover:border-[#525252] transition-colors bg-transparent group">
          <div className="text-2xl mb-4">{c.icon}</div>
          <p className="text-xs font-mono tracking-[0.15em] uppercase text-[#525252] mb-2 group-hover:text-[#3B82F6] transition-colors">{c.title}</p>
          <p className="text-sm text-[#A3A3A3] mb-6 leading-relaxed">{c.desc}</p>
          <ul className="space-y-1">
            {c.tags.map((t) => (
              <li key={t} className="text-xs font-mono text-[#525252] flex items-center gap-2">
